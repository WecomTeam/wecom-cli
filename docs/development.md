...
| `crates/wecom-cli/` | 二进制（bin）：`main.rs` 组装 Client 并 `run`；`auth` 鉴权体系（bot 凭据/扫码/签名引导/凭据加密/`AuthSession` 运行时会话）；`WecomBackend`（持有 token 即注入 Bearer token、`RequireAuth` 前置门禁，853004 静默刷新委托 `AuthSession`）；config/env/logging；`auth` 命令经扩展命令点挂载 |
